...
class PositionController:
    def __init__(self):
        ...
        self.zero_offset = [-900.0, 2100.0]  # 设置手动零点
    ...
    def setup_movement_controls(self):
        ...
        keyboard.add_hotkey('z', self.set_current_position_as_zero)  # 按 'z' 键设置当前位置为零点
    ...
    def set_current_position_as_zero(self):
        # 设定新的零点
        print(f"将当前位置 X{self.current_position[0]} Y{self.current_position[1]} 设置为零点")
        self.zero_offset = self.current_position.copy()  # 将当前位置设置为零点，而不是固定的 [-900.0, 2100.0]
        self.current_position = [0, 0]
        print(f"已将当前位置设置为新的零点: X{self.zero_offset[0]} Y{self.zero_offset[1]}")
    ...
    def execute_move(self, x, y):
        try:
            ...
            # 计算实际绝对坐标
            absolute_x = self.current_position[0] + self.zero_offset[0]
            absolute_y = self.current_position[1] + self.zero_offset[1]
